...
function lockEscrow() {
  ...
  if (!agreed) { pushNotif('⚠️','','Acepta los términos primero.','#FEF3C7'); return; }
  ...
  const gig = {
    ...
    status:     'working',  // escrow|working|delivered|done
    progress:   30,
    messages:   [
      { from:'system', text:`🔒 ${activeTicket.ch} CH bloqueados en escrow. Puedes empezar.`, time: now() },
      { from:'expert', text:`¡Hola! Vi los detalles de tu proyecto de ${activeTicket.category}. Ya mismo empiezo. 💪`, time: now() },
    ],
  ...
  pushNotif('🔒','¡Escrow activado!',`${activeTicket.ch} CH congelados. ${selectedExpert.name} fue notificado.`,'#D1FAE5');
  pushNotif('👷','Trabajo iniciado',`${selectedExpert.name}: "Los CH están asegurados. ¡Empezando ahora!"`, '#DBEAFE');

  // Refresh tasks if visible
  if (document.getElementById('secTasks').classList.contains('active')) renderTasks();

  // Simulate progress after 5s
  setTimeout(() => simulateProgress(gig.id), 5000);
}
